...
12. **데이터베이스 이름** 은 **workshop** 를 입력하고 {{% button href="#" %}}생성{{% /button %}} 버튼을 클릭합니다.
...
16. 크롤링 할 S3 버킷 및 폴더를 지정합니다. 포함 경로 에 **s3://analytics-workshop-[개인식별자]/source** 을 입력하거나 탐색기 버튼을 클릭하여 **analytics-workshop-[개인식별자]** 버킷 아래 **source** 폴더를 {{% button href="#" %}}선택{{% /button %}} 한 후 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
...
20. 데이터베이스는 앞서 생성한 **workshop**를 선택한 후 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
...
25. Kinesis Firehose가 **s3://analytics-workshop-[개인식별자]/source** 에 저장한 JSON 파일 포맷 스트림 데이터를 Parquet 파일 포맷으로 변경하고, 일부 컬럼의 데이터 타입을 변경한 후 **s3://analytics-workshop-[개인식별자]/parquet** 폴더에 파일을 저장하는 Glue ETL 작업을 생성하겠습니다.
26. 우선 ETL후 결과 파일이 저장될 폴더를 생성합니다. S3 콘솔로 로그인한 후 **analytics-workshop-[개인식별자]** 버킷를 선택하고 **parquet** 폴더를 생성합니다.
...
30. **변환 유형 선택** 화면에서 **스키마 변경**을 선택하고 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
31. **데이터 대상 선택** 화면에서 **데이터 대상에서 테이블 생성**을 선택하고 **데이터 스토어**는 **Amazon S3**, **형식**은 **Parquet**, **대상 경로**는 **s3://analytics-workshop-[개인식별자]/parquet** 을 입력합니다. {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
![image08](images/08.png)
32. **occurencestartdate**, **discoverydate** 컬럼의 데이터 형식을 date로 변경한 후 {{% button href="#" %}}작업 저장 및 스크립트 편집{{% /button %}} 을 클릭합니다.
![image09](images/09.png)
33. 스크립트 내용을 검토한 후 상단의 {{% button href="#" %}}작업 실행{{% /button %}} 버튼을 클릭하여 ETL을 시작합니다. 작업 완료까지 수 분이 소요될 수 있습니다.
34. Glue ETL 작업이 끝나면 **s3://analytics-workshop-[개인식별자]/parquet** 폴더에 **parquet** 타입 파일이 생성됩니다.
35. Glue ETL이 변환한 Parquet 파일을 크롤링하여 테이블을 생성합니다. **AWS Glue** > **크롤러** > **TestCrawler**를 선택하고 **작업** > **크롤러 편집** 를 선택합니다.
![image10](images/10.png)
36. **크롤러 정보 추가** 화면에서 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
37. **Specify crawler source type** 화면에서 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
38. **데이터 스토어 추가** 화면에서 {{% button href="#" %}}다음{{% /button %}} 을 를 클릭합니다.
39. **다른 데이터 스토어 추가** 화면에서 **예**를 선택한 후 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
40. **데이터 스토어 추가** 화면에서 포함 경로에 **s3://analytics-workshop-[개인식별자]/parquet** 을 입력한 후 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
![image11](images/11.png)
41. **다른 데이터 스토어 추가** 화면에서 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
42. **IAM 역할 선택** 화면에서 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
43. **이 크롤러의 일정 생성** 화면에서 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
44. **크롤러의 출력 구성** 화면에서 {{% button href="#" %}}다음{{% /button %}} 을 클릭합니다.
45. **리뷰** 화면에서 {{% button href="#" %}}마침{{% /button %}} 을 클릭합니다.
46. **TestCrawler**를 선택하고 {{% button href="#" %}}크롤러 실행{{% /button %}} 을 클릭합니다.
![image12](images/12.png)
47. **크롤러** 실행 후 **parquet** 테이블이 생성됩니다.
...
4. 쿼리 결과 위치에 **s3://analytics-workshop-[개인식별자]/query-result/** 를 입력하고 {{% button href="#" %}}저장{{% /button %}} 을 클릭합니다.
...
4. **QuickSight 리전**은 **US-West (Oregon)**을 선택하고 **QuickSight 계정 이름**은 **임의로 지정(중복될 경우 계정이 생성되지 않습니다)** 하고 **알림 이메일 주소**는 개인 Email 주소를 입력합니다. QuckSight가 S3에 접근해야 하므로, **Choose S3 buckets**를 클릭하여 **analytics-workshop-[개인식별자]** 을 선택한 후 {{% button href="#" %}}완료{{% /button %}} 를 클릭합니다.
...
8. Athena를 클릭하고 팝업 창의 **데이터 원본 이름**에 **analytics-quicksight**를 입력(임의의 값 입력 가능)하고 {{% button href="#" %}}데이터 원본 생성{{% /button %}} 버튼을 클릭합니다.
![image21](images/21.png)
9. **테이블 선택**에서 **데이터베이스**는 **workshop**, **테이블**은 **parquet**를 선택하고 {{% button href="#" %}}선택{{% /button %}} 버튼을 클릭합니다.
